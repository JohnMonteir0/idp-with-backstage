...
The Deployment follows Backstage's Kubernetes guidance: PostgreSQL runs as a separate Deployment with a Service and PVC, `postgres-secrets` supplies its credentials, and `backstage-secrets` supplies the GitHub token and portal URL. This example uses the in-cluster hostname `postgres.backstage` and an EKS `gp3` PVC. The committed values are demo values only; replace `GITHUB_TOKEN` and protect both Secrets with SealedSecrets or ExternalSecrets before production use.
