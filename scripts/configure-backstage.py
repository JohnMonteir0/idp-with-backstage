...
def configure(root, env):
    ...
    tls_secret = required('TLS_SECRET_NAME', r'[a-z0-9]+(?:[.-][a-z0-9]+)*')
    ...
    container['env'].append({'name': 'BACKSTAGE_BASE_URL', 'value': f'https://{hostname}'})
    ...
    ingress['spec']['tls'] = [{'hosts': [hostname], 'secretName': tls_secret}]
